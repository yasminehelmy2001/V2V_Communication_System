...
/* we are using pipe 0, so tx and pipe 0 address must be the same ! That's it ! Very simple !
 * tx uses pipe 0 address to send to receiver
 * and receiver uses pipe 0 address to send ack to transmitter
 * so both need the same address
